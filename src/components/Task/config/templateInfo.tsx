...
export const buildINFO = ({ classes, info, key, isIncident, isInWork }: any) => {
  const build: any = {
    ...
    po: (key: any) => <Typography variant="h4">{info[key]}</Typography>,
    uri: (key: any) => <Typography variant="h4">{info[key]}</Typography>,
    taskCreator: (key: any) => <Typography variant="h4">{info[key]}</Typography>,
  };
